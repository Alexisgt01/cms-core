                    ...
                    FileUpload::make('src')
                        ...
                        ->afterStateUpdated(function (TemporaryUploadedFile $state, callable $set) {
                            ...
                            if ($dimensions = $state->dimensions()) {
                                ...
                                $ratio = $dimensions[0] > 0 ? $dimensions[1] / $dimensions[0] : 0;
                        ...
                        ->saveUploadedFileUsing(function (BaseFileUpload $component, TemporaryUploadedFile $file): string {
                            ...
                            $media = app(MediaService::class)->storeUploadedFile($uploadedFile);
                    ...
                    Group::make([
                        TextInput::make('width')
                            ...
                            ->afterStateUpdated(function (callable $get, callable $set, ?string $state): void {
                                $ratio = (float) $get('aspect_ratio');
                                if ($ratio > 0 && $state !== null && $state !== '') {
                                    $set('height', (int) round((int) $state * $ratio));
                                }
                            }),
                        TextInput::make('height')
                            ...
                            ->afterStateUpdated(function (callable $get, callable $set, ?string $state): void {
                                $ratio = (float) $get('aspect_ratio');
                                if ($ratio > 0 && $state !== null && $state !== '') {
                                    $set('width', (int) round((int) $state / $ratio));
                                }
                            }),
                    ])->columns(),
                    Hidden::make('type')
                        ->default('document'),
                    Hidden::make('aspect_ratio')
                        ->default(0),
                ];
