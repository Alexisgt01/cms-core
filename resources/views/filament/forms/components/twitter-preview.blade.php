<div
    x-data="{
        get cardType() {
            return $wire.data?.twitter_card || '{{ \Alexisgt01\CmsCore\Models\BlogSetting::instance()->twitter_card_default ?? 'summary_large_image' }}'
        },
        get title() {
            return $wire.data?.twitter_title || $wire.data?.og_title || $wire.data?.meta_title || $wire.data?.title || $wire.data?.name || $wire.data?.display_name || ''
        },
        get description() {
            return $wire.data?.twitter_description || $wire.data?.og_description || $wire.data?.meta_description || ''
        },
        get site() {
            return $wire.data?.twitter_site || '{{ \Alexisgt01\CmsCore\Models\BlogSetting::instance()->twitter_site ?? '' }}'
        },
        get imageUrl() {
            const img = $wire.data?.twitter_image || $wire.data?.og_image
...
>
    <span class="mb-3 block text-sm font-medium text-gray-700 dark:text-gray-300">Apercu Twitter / X</span>
    ...
    <div x-show="cardType === 'summary_large_image'" class="max-w-[504px] overflow-hidden rounded-xl border border-gray-300 dark:border-gray-600">
        ...
        <div class="bg-white px-3 py-2.5 dark:bg-gray-800">
            <div class="text-base font-bold leading-tight text-gray-900 dark:text-gray-100" x-text="truncate(title || 'Titre Twitter...', 70)" :class="!title && 'text-gray-300 dark:text-gray-600'"></div>
            <div class="mt-0.5 line-clamp-2 text-sm text-gray-500 dark:text-gray-400" x-text="truncate(description || 'Description Twitter...', 200)" :class="!description && 'text-gray-300 dark:text-gray-600'"></div>
